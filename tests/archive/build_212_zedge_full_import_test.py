#!/usr/bin/env python3
"""role212 z EDGE (z=+30.5) full standalone import test: clone export 2139,
regenerate ALL 8 chunks (main + spanning + cz=3 boundaries + empties)."""
...
def main():
    # 2139: x=+1.5,+2.5 (Nc2), y=-1.5,-2.5 (ly1,Ye2), z=+29.5,+30.5 (lz_near=29,lz_far=30,edge)
    with open(find_export(2139)) as f:
        bp = json.load(f)
    ...
    for e in bp['VoxelData']:
        ...
        if cx == 1:
            ...
            gen, gmc = g.generate_212_dense_scan(1, 1, 2, 2, 29, 30)
        elif (cx, cy, cz) == (2, 1, 3):
            gen, gmc = g.generate_212_zedge_213_scan(2, 2)
        elif (cx, cy, cz) == (2, 2, 2):
            gen, gmc = g.generate_212_spanning_222_scan(1, 2, 29, 30)
        elif (cx, cy, cz) == (2, 2, 3):
            gen, gmc = g.generate_212_zedge_223_scan(2)
        ...
        n += 1
    assert n == 8, n
    out = os.path.join(HERE, '212_zedge_full_import_test.blueprint')
    with open(out, 'w') as f:
        json.dump(bp, f)
    print(f'role212 z edge full ({n} chunks generated): sanity OK. Written: {out}')
