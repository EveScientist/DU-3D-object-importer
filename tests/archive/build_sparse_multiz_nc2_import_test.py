#!/usr/bin/env python3
"""
Build a real test blueprint for sparse multi-z K=2 + Nc=2 (two columns,
each with a single gap between z=0.5 and z=14.5) in-game import validation
-- the Ncols generalization of the earlier Nc=1 case. Clone real export
1903 verbatim, and replace all 4 h3 entries' voxel.data with bytes
produced entirely by h3_generator.py's generate_sparse_multiz_scan.
"""
import sys
import os
import json
...
def main():
    ...
    roles = {
        (2, 1, 2): ('212', 1, 686, 29), (2, 2, 2): ('222', 161, 526, 19),
    ...
    for e in bp['VoxelData']:
        ...
        if key not in roles:
            continue
        role, own, mc_nc2, pos1 = roles[key]
        real_blob = base64.b64decode(e['records']['voxel']['data']['$binary'])
        real_scan, real_mc = decode_chunk_from_blob(real_blob)

        gen_scan, gen_mc = g.generate_sparse_multiz_scan(role, own, mc_nc2, 0, 14, pos1, Ncols=2)
    ...
    out_path = os.path.join(HERE, 'sparse_multiz_nc2_import_test.blueprint')
    with open(out_path, 'w') as f:
        json.dump(bp, f)
    print(f'Sparse multi-z K=2 + Nc=2: sanity OK, {n_replaced} h3 entries substituted (from export {export_num}).')
