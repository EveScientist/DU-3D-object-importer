#!/usr/bin/env python3
"""role222 pos-y EDGE (y=+30.5) full standalone import test: clone export 2187,
...
def main():
    # 2187: x=+1.5,+2.5 (Nc2), y=+1.5,+2.5 (Ye2), z=+29.5,+30.5 (edge)
    with open(find_export(2187)) as f:
        bp = json.load(f)
    ...
    for e in bp['VoxelData']:
        ...
        if (cx, cy, cz) == (2, 2, 2):
            gen, gmc = g.generate_222_dense_scan(1, 1, 2, 2, 29, 30)
        elif (cx, cy, cz) == (2, 2, 3):
            gen, gmc = g.generate_222_zedge_223_scan(1, 1, 2, 2)
        else:
        ...
        n += 1
    assert n == 8, n
    out = os.path.join(HERE, '222_zedge_full_import_test.blueprint')
    with open(out, 'w') as f:
        json.dump(bp, f)
    print(f'role222 z edge full ({n} chunks generated): sanity OK. Written: {out}')
