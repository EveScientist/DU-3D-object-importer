#!/usr/bin/env python3
"""role112 z EDGE (z=+30.5) full standalone import test: clone export 2240,
...
def main():
    # 2240: x=-5.5,-6.5 (lx5 deep), y=-30.5,-31.5 (ly_FAR=31 edge), z=+1.5,+2.5
    with open(find_export(2240)) as f:
        bp = json.load(f)
    ...
    for e in bp['VoxelData']:
        ...
        if (cx, cy, cz) == (1, 1, 2):
            gen, gmc = g.generate_112_dense_scan(5, 5, 2, 2, 29, 30)
        elif (cx, cy, cz) == (1, 1, 3):
            gen, gmc = g.generate_112_zedge_113_scan(5, 5, 2, 2)
        else:
        ...
        n += 1
    assert n == 2, n
    out = os.path.join(HERE, '112_zedge_full_import_test.blueprint')
    with open(out, 'w') as f:
        json.dump(bp, f)
    print(f'role112 z edge full ({n} chunks generated): sanity OK. Written: {out}')
