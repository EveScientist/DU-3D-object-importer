#!/usr/bin/env python3
"""
Build a real test blueprint for the Z negative far edge (z=-31.5, isolated)
in-game import validation. Clone real export 1750 verbatim, replace its 8
h3 entries' voxel.data with bytes produced entirely by h3_generator.py's
generate_zneg_faredge_pair_scan.
"""
import sys
import os
import json
...
def main():
    ...
    roles = [('212', 2, 1, 22, 29), ('222', 2, 2, 182, 19),
             ('112', 1, 1, 54, 335), ('122', 1, 2, 214, 325)]

    n_replaced = 0
    for role, cx, cy, marker_cz1, pos1 in roles:
        ...
        assert cz1_scan == real_scan1, f'cz=1 {role} scan mismatch'
        replace_h3_entry(bp, (cx, cy, 1), cz1_scan, real_mc1)
        n_replaced += 1

        real_scan0, real_mc0 = decode_chunk_from_blob(find_blob(cx, cy, 0))
        assert cz0_scan == real_scan0, f'cz=0 {role} scan mismatch'
        assert gen_mc0 == real_mc0
        replace_h3_entry(bp, (cx, cy, 0), cz0_scan, real_mc0)
        n_replaced += 1

    assert n_replaced == 8, f'expected 8, got {n_replaced}'
    out_path = os.path.join(HERE, 'faredge_zneg_import_test.blueprint')
