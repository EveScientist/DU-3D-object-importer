#!/usr/bin/env python3
"""
Build a real test blueprint for multi-z + Nc=2 + Ye=3 (a 2x3-column, full
z=0..14 tower) in-game import validation -- the Ye=3 generalization of the
earlier Ye=2 case. Clone real export 1899 verbatim, and replace all 8 h3
entries' voxel.data with bytes produced entirely by h3_generator.py's
generate_multiz_nc_ye_212family_scan/generate_multiz_nc_ye_222family_scan
(cz=2) and generate_cz1_nc_ye_212family_scan/generate_cz1_nc_ye_222family_scan
(cz=1).
"""
import sys
import os
...
def main():
    ...
    # 212 family
    for role, cx, cy, own, mc_singlez, base_cz1_mc, pos1 in [
            ('212', 2, 1, 187, 686, 578, 27), ('112', 1, 1, 219, 599, 546, 333)]:
        real_scan2, real_mc2 = decode_chunk_from_blob(find_blob(cx, cy, 2))
        gen_scan2, gen_mc2 = g.generate_multiz_nc_ye_212family_scan(
            role, Ncols, own, mc_singlez, 0, 14, Ye, pos1)
        ...
        gen_scan1, gen_mc1 = g.generate_cz1_nc_ye_212family_scan(
            role, role == '212', Ncols, cz2_marker1, base_cz1_mc, Ye, pos1)
    ...
    # 222 family
    for role, cx, cy, own, mc_singlez, base_cz1_mc, pos1 in [
            ('222', 2, 2, 161, 526, 674, 19), ('122', 1, 2, 193, 695, 642, 325)]:
        real_scan2, real_mc2 = decode_chunk_from_blob(find_blob(cx, cy, 2))
        ...
        cz2_marker1 = (own + 14 + 0 + 7 - 35 * min(Ye - 1, 1)) % 256
        real_scan1, real_mc1 = decode_chunk_from_blob(find_blob(cx, cy, 1))
        gen_scan1, gen_mc1 = g.generate_cz1_nc_ye_222family_scan(
            role, role == '222', Ncols, cz2_marker1, base_cz1_mc, pos1)
        assert gen_scan1 == real_scan1, f'cz=1 {role} scan mismatch'
        assert gen_mc1 == real_mc1
        replace_h3_entry(bp, (cx, cy, 1), gen_scan1, real_mc1)
        n_replaced += 1
    ...
    out_path = os.path.join(HERE, 'multiz_nc_ye3_import_test.blueprint')
    with open(out_path, 'w') as f:
        json.dump(bp, f)
    print(f'Multi-z+Nc=2+Ye=3: sanity OK, {n_replaced} h3 entries substituted (from export {export_num}).')
