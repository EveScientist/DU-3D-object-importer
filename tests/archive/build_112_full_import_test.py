#!/usr/bin/env python3
"""role112 (neg-x/neg-y) full interior import test: clone export 2197,
regenerate ALL 8 chunks (main + 3 spanning + 4 empties) from scratch."""
...
def main():
    # 2197: x=-1.5,-2.5 (Nc2), y=-1.5,-2.5 (Ye2), z=+1.5,+2.5 (N2) -- touches both center planes
    with open(find_export(2197)) as f:
        bp = json.load(f)
    ...
    for e in bp['VoxelData']:
        ...
        if (cx, cy, cz) == (1, 1, 2):
            gen, gmc = g.generate_112_dense_scan(1, 1, 2, 2, 1, 2)
        elif (cx, cy, cz) == (2, 1, 2):
            gen, gmc = g.generate_112_xspan_212_scan(1, 2, 1, 2)
        elif (cx, cy, cz) == (1, 2, 2):
            gen, gmc = g.generate_112_yspan_122_scan(1, 2, 1, 2)
        elif (cx, cy, cz) == (2, 2, 2):
            gen, gmc = g.generate_112_corner_222_scan(1, 2)
        else:
        ...
        n += 1
    assert n == 8, n
    out = os.path.join(HERE, '112_full_import_test.blueprint')
    with open(out, 'w') as f:
        json.dump(bp, f)
    print(f'role112 full interior ({n} chunks generated): sanity OK. Written: {out}')
