#!/usr/bin/env python3
"""
Build a real test blueprint for multi-z stacking + Nc=2 (two columns, full
z tower lz=0..14) in-game import validation. Clone real export 1636
verbatim, and replace all 8 h3 entries' voxel.data with bytes produced
entirely by h3_generator.py's generate_multiz_nc2_scan/generate_cz1_nc2_scan
(for the Nc-tracking '212'/'222' roles) and generate_multiz_scan/
generate_cz1_scan (for the boundary '112'/'122' roles, unaffected by Nc).
"""
import sys
import os
import json
...
def main():
    ...
    for role, cx, cy, base_own, base_mc_nc2, base_cz1_mc, pos1 in [
        ...
        gen_scan2, gen_mc2 = g.generate_multiz_nc2_scan(role, base_own, base_mc_nc2, 0, 14, pos1)
        ...
        gen_scan1, gen_mc1 = g.generate_cz1_nc2_scan(role, cz2_marker1, base_cz1_mc, pos1)
    ...
    # boundary roles, unaffected by Nc -- reuse plain multi-z generators
    for role, cx, cy, base_own, base_mc, pos1 in [
            ('112', 1, 1, 33, 599, 335), ('122', 1, 2, 193, 695, 325)]:
        real_scan2, real_mc2 = decode_chunk_from_blob(find_blob(cx, cy, 2))
        gen_scan2, gen_mc2, first_marker = g.generate_multiz_scan(role, base_own, base_mc, 0, 14, pos1)
        assert gen_scan2 == real_scan2, f'cz=2 ({cx},{cy}) scan mismatch'
        assert gen_mc2 == real_mc2, f'cz=2 ({cx},{cy}) mc mismatch'
        replace_h3_entry(bp, (cx, cy, 2), gen_scan2, real_mc2)
        n_replaced += 1
    ...
    out_path = os.path.join(HERE, 'multiz_nc2_import_test.blueprint')
    with open(out_path, 'w') as f:
        json.dump(bp, f)
    print(f'Multi-z+Nc=2: sanity OK, {n_replaced} h3 entries substituted (from export {export_num}).')
