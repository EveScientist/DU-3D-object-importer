#!/usr/bin/env python3
"""
Build a real test blueprint for K=3 sparse multi-z with multi-level
sub-runs (segment widths 2/1/2: z=10-11, gap, z=13, gap, z=15-16) in-game
import validation -- the multi-level generalization of
generate_multigap_multiz_scan. Clone real export 1915 verbatim, and
...
def main():
    ...
    for e in bp['VoxelData']:
        ...
        gen_scan, gen_mc = g.generate_multigap_multiz_scan(role, own, mc_singlez, [(10, 11), 13, (15, 16)], pos1)
    ...
    out_path = os.path.join(HERE, 'multigap_widths_import_test.blueprint')
    with open(out_path, 'w') as f:
        json.dump(bp, f)
    print(f'K=3 multigap, multi-level widths: sanity OK, {n_replaced} h3 entries substituted (from export {export_num}).')
