...
# ── x=0 seam ────────────────────────────────────────────────────────────────
for _num, _nr, _h in [(3032, 2, 1), (3036, 3, 1), (3040, 2, 2), (3042, 2, 3)]:
...
for _num, _hc, _ho, _lc, _lo in [
        ...
        (3058, [1, 2], [1, 2], [1, 2], [1, 2])]:           # valley at boundary
...
def _(num=3044):
    c = chunks(num)
    assert D.gen_seam_y0_high(2, h=2) == c[HIGH_Y], "HIGH"
...
def _():
    for nr, h in [(2, 1), (2, 2), (2, 3)]:
        u = [h] * nr
        assert D.gen_seam_y0_high_varying(u, [h, h]) == \
            D.gen_seam_y0_high(nr, h=h), f"HIGH n{nr} h{h}"
        assert D.gen_seam_y0_low_varying(u, [h, h]) == \
