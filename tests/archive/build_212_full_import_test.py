#!/usr/bin/env python3
"""role212 (pos-x/neg-y) full standalone import test: clone export 2101,
regenerate ALL 8 chunks (dense main + spanning + 6 empties) from scratch."""
...
def main():
    # 2101: x=+1.5,+2.5 (lx_near=1,Nc2), y=-1.5,-2.5 (ly_near=1,Ye2), z=+1.5,+2.5 (N2)
    with open(find_export(2101)) as f:
        bp = json.load(f)
    ...
    for e in bp['VoxelData']:
        ...
        if (cx, cy, cz) == (2, 1, 2):
            gen, gmc = g.generate_212_dense_scan(1, 1, 2, 2, 1, 2)
        elif (cx, cy, cz) == (2, 2, 2):
            gen, gmc = g.generate_212_spanning_222_scan(1, 2, 1, 2)
        else:
        ...
        n += 1
    assert n == 8, n
    out = os.path.join(HERE, '212_full_import_test.blueprint')
    with open(out, 'w') as f:
        json.dump(bp, f)
    print(f'role212 full ({n} chunks generated): sanity OK. Written: {out}')
