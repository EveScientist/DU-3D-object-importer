#!/usr/bin/env python3
"""
Build a real test blueprint for multi-z stacking in-game import validation.
Same clone-and-substitute approach as the X/Y/Z-spanning import tests (all
deployed successfully on 2026-06-19): clone real export 1628 (a tower of
voxels from z=-0.5 up to z=+14.5, lz=0..14, all 15 levels, single x/y column)
verbatim, and replace only the 8 h3 entries' voxel.data with bytes produced
entirely by h3_generator.py's generate_multiz_scan/generate_cz1_scan.
"""
import sys
import os
import json
...
def main():
    ...
    out_path = os.path.join(HERE, 'multiz_import_test.blueprint')
    with open(out_path, 'w') as f:
        json.dump(bp, f)
    print(f'Multi-z: sanity OK, {n_replaced} h3 entries substituted (from export {export_num}).')
