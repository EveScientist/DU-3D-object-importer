#!/usr/bin/env python3
"""
Build a real test blueprint for the cross-chunk sign-crossing
offset/staggered case (col1 y=-0.5 in cy=1, col2 y=+0.5 + col3 y=+1.5
together in cy=2) in-game import validation. Clone real export 1925
verbatim, and replace the '212' (cy=1, unchanged from the established
Ncols=2 formula) and '222' (cy=2, new generate_offset_crosschunk_other_scan
with n_extra=2) h3 entries' voxel.data with bytes produced entirely by
h3_generator.py.
"""
import sys
import os
...
def main():
    ...
    for e in bp['VoxelData']:
        ...
        if key == (2, 1, 2):
            ...
            gen_scan = g.generate_offset_212family_scan('212', 1, 199, -1, real_mc, 29)
        elif key == (2, 2, 2):
            real_blob = base64.b64decode(e['records']['voxel']['data']['$binary'])
            real_scan, real_mc = decode_chunk_from_blob(real_blob)
            gen_scan = g.generate_offset_crosschunk_other_scan('222', 161, 2, real_mc, 19)
        else:
            continue
    ...
    out_path = os.path.join(HERE, 'offset_crosschunk_import_test.blueprint')
    with open(out_path, 'w') as f:
        json.dump(bp, f)
    print(f'Offset cross-chunk sign-crossing: sanity OK, {n_replaced} h3 entries substituted (from export {export_num}).')
