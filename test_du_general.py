...
def flatcols(hplanes, z0=8):
    return {(x,y):(z0,z0+h-1) for x,row in enumerate(hplanes) for y,h in enumerate(row) if h>0}
def surfcols(zlo,zhi):
    return {(x,y):(zlo[x][y],zhi[x][y]) for x in range(len(zlo)) for y in range(len(zlo[0]))}
def narrowcols(sets,H,z0=8):
    return {(x,y):(z0,z0+H-1) for x,s in enumerate(sets) for y in s}
...
tests=[
 ...
 ('3361 OVH1',       {**{(x,y):(8,9) for x in range(3) for y in range(3)},
                      **{(x,1):[(8,9),(12,13)] for x in range(3)}}),
 ('3363 OVH2',       {**{(x,y):(8,9) for x in range(3) for y in range(3)},
                      **{(x,1):[(8,9),(13,15)] for x in range(3)}}),
]
allok=True
