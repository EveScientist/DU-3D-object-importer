...
- **tests/dep23b_sphere.blueprint** (2026-07-18) — DEPLOYMENT 23b: single M smooth sphere, EMPTY-CELLS FIX. dep23 placed the core right but rendered no voxels: the centering offset moved content off the chunks the LOD set named. FIX: octree_from_cells derives the h3 chunk set from the ACTUAL placed cells (vox_abs), not chunk0+voxel//32. Now non-empty (46737 owned voxels), centered; + material-ownership LOD fix (chunk = (voxel+1)//32) so edge voxels are owned. AWAITING DEPLOY.

- **DEPLOYMENT 24 (dep24_half_A/B) rebuilt** (2026-07-18): material-ownership LOD fix closes the 1-voxel seam gap. Half A owns x-cells 113..127, half B 0..14; when core B grid-aligned +1 core (+X), geometric seam gap=0. DEPLOYED: reconstruction better than dep22 but residual seam artifacts (content renders ~half a grid beyond each core edge = material+1 boundary layer; over/under-size + hairline holes = DU meshing each core INDEPENDENTLY). CONCLUSION: separate-core seams are DU-limited (no cross-core mesh weld); single-core is the clean path.

- **tests/dep25_sphere_hires.blueprint** (2026-07-18) — DEPLOYMENT 25: HIGH-RES sphere (M, grid 143 = 4x the old default via true BUILD=vsz*4 resolution). 1.5M voxels, 149 records, 737KB. Emitter now numpy-vectorized + per-chunk bucketed (72s->11s emit at grid128). DEPLOYED: sharp but FACETED (~12-vox flat faces) -- smoothing projected onto the low-poly (768-tri) mesh triangles. Fixed by PN-triangle projection; see dep26.

- **tests/dep26_sphere_pn.blueprint** (2026-07-18) — DEPLOYMENT 26: hi-res sphere with PN-TRIANGLE (smooth-normal) projection. dep25 faceted because smoothing projected onto flat low-poly triangles. Now anchors project onto the vertex-normal PN surface -> radial error 0.35->0.009 vox vs the true sphere. Grid 143, 954KB, 40s. AWAITING DEPLOY (should be a genuinely round sphere).

- **LOD CUBE SWEEP (LODSW)** (2026-07-19) — LOW-corner: **3824** (C1 1³), **3826** (C2 2³), **3829** (C5 5³), **3831** (C6 6³), **3833** (C8 8³), **3835** (C16 16³). HIGH-corner mirror (+96 all axes → h3 chunk 11): **3837** (H1 1³), **3839** (H2 2³), **3841** (H5 5³), **3843** (H8 8³). All hcCarbon, Static M, single box-fill. Full analysis in PROBES_0710d.md. **Findings: (1) h3+h4 always minimal ancestor, h7=root never expands; expansion is always the 2×2×2 block {a_L−1,a_L}³ (low corner) or {a_L,a_L+1}³ (high corner). (2) Neighbour DIRECTION is position-dependent — CONFIRMED by the mirror (low→lower nbr, high→upper nbr). (3) The earlier N≥2^(L−4) threshold is REFUTED — it was a low-corner artifact (low h5 expands N≥2; high h5 expands only N≥8, same ancestor chunk). Exact threshold NOT yet pinned; needs DU create_lods source or a denser sweep.** Shell is DU's export form, not an import requirement (minimal ancestor tree still deploys) — QA-only, low priority.
