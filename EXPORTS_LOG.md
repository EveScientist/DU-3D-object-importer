...
- **tests/dep23b_sphere.blueprint** (2026-07-18) — DEPLOYMENT 23b: single M smooth sphere, EMPTY-CELLS FIX. dep23 placed the core right but rendered no voxels: the centering offset moved content off the chunks the LOD set named. FIX: octree_from_cells derives the h3 chunk set from the ACTUAL placed cells (vox_abs), not chunk0+voxel//32. Now 8/8 h3 records non-empty (mat 6k-9k), chunks 7-8, bounds center 64.2 == element 64.125. AWAITING DEPLOY.
