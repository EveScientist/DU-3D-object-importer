...
| 3081 | Build AV — SMOOTHED 3048 hump (smooth tool on top step edges), 12 vox | same coords as 3048; smooth applied in-game | (8,8,8)+(7,8,8) | SMOOTH-THROUGH-SEAM DECODED (opens END-GOAL arc): smoothing changes ONLY displacement slots — structure/values/runs/decls/pads byte-identical to blocky 3048. Carriers per group form: TRI groups → T2 slot (T1 keeps +14/+42); flat run>0 → 12B two-vertex [val,1,run, 7e-triple, run−1, V1, 0,0] (s-slot = run−1, retro-fits old single-chunk refs); (0,0) fillers → in-place 8B; run-0 content rows stay NEUTRAL. Displacement PER-CLUSTER uniform: bevel clusters (±28,0,−40), hump-top interior (0,0,−16), far/h1 neutral; perfectly mirrored across the seam. apply_seam_displacement coded; BOTH chunks reconstruct byte-exact from gen_seam_x0_*_varying + the map (suite 49/49). REMAINING unknown of the arc: the displacement VALUE function (DU's smoothing vertex solve). |
