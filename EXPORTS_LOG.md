...
- **LOD CUBE SWEEP (LODSW)** (2026-07-19) — Exports **3824** (C1 1³), **3826** (C2 2³), **3829** (C5 5³), **3831** (C6 6³), **3833** (C8 8³), **3835** (C16 16³). All hcCarbon, Static M, single box-fill, content in h3 chunk (8,8,8), standard low-corner placement. Decoded octree LOD sets solve compute_lod_set for single-chunk solids (see PROBES_0710d.md RESULT). Combined with 3259 (3³) / 3240 (4³): **h3,h4 always minimal ancestor; level L≥5 expands to the {a_L−1,a_L}³ lower-corner block iff N ≥ 2^(L−4)** (h5→N≥2, h6→N≥4; N=3/N=4 pin the h6 threshold). h7=root, never expands. Shell is DU's export form, not an import requirement (minimal ancestor tree still deploys).
