...
for entry in bp["VoxelData"]:
    if entry["h"] == 3 and entry["x"]["$numberLong"] == 2 and entry["y"]["$numberLong"] == 2 and entry["z"]["$numberLong"] == 2:
        ...
        new_counter = old_counter + 256  # same low byte, so scan[mat_byte_pos] stays self-consistent
        print(f"old mat_counter={old_counter} (0x{old_counter:x}), new={new_counter} (0x{new_counter:x})")
        assert (old_counter & 0xff) == (new_counter & 0xff)
        ...
        # deliberately leave records.voxel.hash and records.meta UNCHANGED (now stale)
        found = True
assert found

bp["Model"]["Id"] = 1518
bp["Model"]["Name"] = "GEN matcounter-only (stale hash)"

with open("/home/du/tests/1518_matcounter_only.blueprint", "w") as f:
    json.dump(bp, f, separators=(',', ':'))

print("wrote /home/du/tests/1518_matcounter_only.blueprint")
