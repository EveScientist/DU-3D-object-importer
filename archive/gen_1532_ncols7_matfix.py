...
for entry in bp["VoxelData"]:
    if entry["h"] == 3 and entry["x"]["$numberLong"] == 2 and entry["y"]["$numberLong"] == 2 and entry["z"]["$numberLong"] == 2:
        # --- voxel record: GENERATED Ncols=7 content, mat[1] held at 2 ---
        # hypothesis: mat[1] is a fixed per-chunk constant (=2), mat[0] wraps mod 256:
        #   mat_counter = 0x0200 | ((410 + 55*Ncols) & 0xff)
        mat_counter = 0x0200 | ((410 + 55 * 7) & 0xff)  # = 0x021B = 539
        ...
        print(f"scan_len={len(scan)} mat_counter={mat_counter} (0x{mat_counter:04x})")
...
bp["Model"]["Id"] = 1532
bp["Model"]["Name"] = "GEN Ncols=7 (mat[1] held at 2)"

with open("/home/du/tests/1532_ncols7_matfix.blueprint", "w") as f:
    json.dump(bp, f, separators=(',', ':'))

print("wrote /home/du/tests/1532_ncols7_matfix.blueprint")
